...
- カノンの遅延着弾は共通のカード効果本数処理を利用し、着弾先が0でも記録量によって復活できます。
...
本番Rules公開後、匿名Auth未対応または旧書き込み形式のキャッシュはFirestoreへ接続できません。v155修正版では`game.js?v=155d`と`style.css?v=155d`を使用します。新しいHTML・JavaScript・CSSの配信を確認してからRulesを公開してください。
